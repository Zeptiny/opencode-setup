...
Ask up to 3 questions targeting the unknowns that would most change the plan. Use the the `question` tool. Never silently skip the question.
...
After structuring the plan, ask the user how they want to receive it using the the `question` tool. Never silently skip the question.
...
2. **Save to disk AND open for review** — Write the markdown file to disk and review.
