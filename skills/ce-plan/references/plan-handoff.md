...
After document-review completes, present the options using the the `question` tool. Never silently skip the question.
...
- **Start `/ce-work`** -> Invoke the `ce-work` skill via the `skill` tool, passing the plan path as the skill argument. Do not merely tell the user to type `/ce-work` — fire the invocation now so the plan executes in this session.
...
4. If no tracker is configured, ask the user which tracker they use with the the `question` tool.g., Codex edit modes) — not because a schema load is required. Never silently skip. Options: `GitHub`, `Linear`, `Skip`. Then:
