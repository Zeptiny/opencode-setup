...
Create a task list of all **new** unresolved items grouped by type (e.g., `todowrite`):
...
1. **Run the project's validation command** (test suite, type check, or whatever the repo's AGENTS.md/AGENTS.md specifies). Run once, not per-agent.
...
Use the platform's blocking question tool: the `question` tool ( Use it to present the decisions and wait for the user's response. After they decide, process the remaining items: fix the code, compose the reply, post it, and resolve the thread.

Fall back to presenting the decisions in the summary output and waiting in conversation only when no blocking tool exists in the harness or the call errors — not because a schema load is required. Never silently skip. If the user doesn't respond, the items remain open on the PR for later handling.
