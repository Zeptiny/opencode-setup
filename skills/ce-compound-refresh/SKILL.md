...
- Ask questions **one at a time** — use the the `question` tool.g., Codex edit modes) — not because a schema load is required. Never silently skip the question
...
Always present choices using the the `question` tool. Never silently skip the question.
