...
If not installed, inform the user that `agent-browser` is not installed. Then stop — this skill cannot function without agent-browser.
...
If not installed, inform the user that `agent-browser` is not installed. Then stop.
...
**Manual mode:** Ask the user whether to run headed or headless using the the `question` tool.g., Codex edit modes) — not because a schema load is required. Never silently skip the question:
