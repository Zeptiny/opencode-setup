...
"Thinking partner" framing does not mean "conversational prose." The parent skill's Interaction Rules apply in full: one question per turn, and use the `question` tool (with its free-text fallback) even for opening and elicitation.
...
**Then offer next steps** using the the `question` tool. Never silently skip the question.
