...
Use prose for the user response (no `question` menu). The justification is Interaction Rule 5(a) in SKILL.md — option sets bias the answer by signaling which dimensions matter.
...
When the soft-cut fires, use the the `question` tool with two options:
