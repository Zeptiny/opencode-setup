...
7. **Spec document review** (optional, for complex specs) — dispatch spec-document-reviewer subagent
...
For complex specs, dispatch a `spec-document-reviewer` subagent to verify completeness and consistency before the user review gate.

Use the `task` tool (`subagent_type: spec-document-reviewer`) with the following base template:
...
If issues are found, fix them inline and optionally re-dispatch the reviewer.
