...
|---|---|---|
| Save | `docs/ideation/YYYY-MM-DD-<topic>-ideation.md` | File save |
| Share | File save | File save |
...
Proof is not available. Skip this path and use file save (5.1) instead.

## Phase 6: Refine or Hand Off

Ask what should happen next using the the `question` tool. Never silently skip the question.
...
3. **Save and end** — persist the ideation using the mode default (file in repo mode), then end.
...
- Write or update the durable record per the mode default in Phase 5 (file in repo mode).
...
Persist via the mode default (5.1 in repo mode), then end. If the user instead asked to use a custom path, honor that explicit request.
