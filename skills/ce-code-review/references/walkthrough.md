...
Each finding is presented in two parts: a **terminal output block** carrying the explanation, and a **question** via the the `question` tool carrying the decision. Never merge the two — the terminal block uses markdown; the question uses plain text.

md` Fall back to numbered options when the `question` tool is unavailable . Never silently skip the question.
...
Fall back to presenting the options as a numbered list and waiting for the user's next reply.
