...
After the preview body is rendered, ask the user using the the `question` tool. The `question` tool is always available in OpenCode. Never silently skip the question.
...
Fall back to presenting numbered options and waiting for the user's next reply.
