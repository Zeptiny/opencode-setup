...
Launch 8 parallel sub-agents using the platform's subagent primitive (the `task` tool with `subagent_type: "explore"`), one for each principle. Each agent should:
