...
When context is ambiguous, use the the `question` tool.g., Codex edit modes) — not because a schema load is required. Never silently skip. If the user declines to pick, assume "partial" mode and proceed conservatively.
...
3. **agent-browser CLI** -- if nothing else is available and `agent-browser` is installed, use it. If not installed, inform the user that `agent-browser` is not installed. Then skip to the next option.
