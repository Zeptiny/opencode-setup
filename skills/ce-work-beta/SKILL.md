...
If it shows an unresolved command string, read `.compound-engineering/config.local.yaml` from the repo root using the native file-read tool (e.g., read in OpenCode). If the file does not exist, all settings fall through to defaults.
...
3. **Create Task List** _(skip if Phase 0 already built one, or if Phase 0 routed as Trivial)_
 - Use the platform's task tracking tool (`todowrite`) to break the plan into actionable tasks
 ...
 **Subagent isolation** — give each parallel subagent its own working tree:
 - **OpenCode (`task` tool):** pass `subagent_type` with the desired agent. OpenCode manages worktree isolation automatically when available. Verify `.opencode/worktrees/` is gitignored before relying on this.
 - **Without worktree isolation:** subagents share the orchestrator's directory.
