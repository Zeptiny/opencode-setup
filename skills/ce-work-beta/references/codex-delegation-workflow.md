...
Present a one-time consent warning using the the `question` tool. The consent warning explains:
...
Critical: Background execution should use the appropriate tool parameters or shell `&` suffix, depending on the platform. Ensure the command does not hit the default timeout.
