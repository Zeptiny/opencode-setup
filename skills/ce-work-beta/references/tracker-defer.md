...
1. Read `AGENTS.md` for tracker references. If nothing found, set `tracker_name = null`, `confidence = low`.
...
Fall back to numbered options and waiting for the user's reply (Interactive mode only).
...
The question-tool name varies by platform. In Interactive mode, use the platform's blocking question tool (the `question` tool. Never silently skip the question.
