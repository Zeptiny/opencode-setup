...
4. **Residual Work Gate** (REQUIRED when Tier 2 ran)
 ...
 Ask the user using the platform's blocking question tool (the `question` tool ). Fall back to numbered options in chat only when the harness genuinely lacks a blocking tool. Never silently skip the gate.
